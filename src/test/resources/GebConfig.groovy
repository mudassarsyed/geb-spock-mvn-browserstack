...
driver = {
    ...
    new BrowserStackDriverFactory().create(username, accessKey,['build': 'Geb-Spock-Mvn-Test','name' : 'single', 'browser':'firefox','os': 'Windows','os_version': '8','browser_version': '69','browserstack.local': browserstackLocal,"browserstack.localIdentifier":browserstackLocalIdentifier])
}
   break
case "chrome" :
driver = {
    ...
    def browserstackLocal = System.getenv("BROWSERSTACK_LOCAL");
    def browserstackLocalIdentifier = System.getenv("BROWSERSTACK_LOCAL_IDENTIFIER");
    new BrowserStackDriverFactory().create(username, accessKey,['build': 'Geb-Spock-Mvn-Test','name' : 'single', 'browser':'chrome','os': 'Windows','os_version': '8','browser_version': '76','browserstack.local': browserstackLocal,"browserstack.localIdentifier":browserstackLocalIdentifier])
}
    break
case "local" :
driver = {
